...
def fit_mlb_encoder(df_train: pd.DataFrame, multilabel_cols: List[str] = MULTILABEL_COLS) -> Dict[str, MultiLabelBinarizer]:
    """Fit multi-label binarizer encoders for the multilabel columns"""
    mlb_encoders = {}
...
def transform_mlb_columns(df: pd.DataFrame, mlb_encoders: Dict[str, MultiLabelBinarizer], multilabel_cols: List[str] = MULTILABEL_COLS) -> pd.DataFrame:
    """Transform the multilabel columns using the fitted multi-label binarizer encoders"""
    encoded_dframes = []
    for col in multilabel_cols:
...
def fit_ohe_encoder(df_train: pd.DataFrame, categorical_cols: List[str] = CATEGORICAL_COLS) -> OneHotEncoder:
    """Fit a One Hot Encoder for the categorical columns"""
...
def transform_ohe_columns(df: pd.DataFrame, ohe_encoder: OneHotEncoder, categorical_cols: List[str] = CATEGORICAL_COLS) -> pd.DataFrame:
    """Transform the categorical columns using the fitted one hot encoder"""
    return pd.DataFrame(
...
def build_mlb_cat_matrix(
    df: pd.DataFrame,
    mlb_encoders: Dict[str, MultiLabelBinarizer],
    multilabel_cols: List[str] = MULTILABEL_COLS,
    categorical_cols: List[str] = CATEGORICAL_COLS
) -> Tuple[pd.DataFrame, List[int]]:
    """Build the full encoded feature matrix by combining the multi-label and categorical encodings"""
...
def build_mlb_ohe_matrix(
    ...
    """Build the full encoded feature matrix by combining the multi-label and one hot encodings"""
    mlb_encoded = transform_mlb_columns(df, mlb_encoders, multilabel_cols).reset_index(drop=True)
