...
def main() -> None:
    ...
    st.markdown(
        ...
        It explores whether regulatory outcomes for UK healthcare data breaches can be modelled from the publicly available {DATA_SOURCE_NAME} dataset.
        ...
        The **Explore** screen presents descriptive views of the underlying training data so users can see what the model has learned from.
        """
    )
    ...
    st.markdown(
        ...
        It is not a regulatory advice tool, not a real-time monitoring system and not authoritative.
    ...
    st.markdown(
        f"""
        The Simulator's regulatory outcome prediction comes from a **gradient-boosted tree classifier** ({classifier_meta.get("selected_model", "CatBoost")}) trained on labelled historical breaches in the {DATA_SOURCE_NAME} dataset.
        The model takes the breach characteristics you provide as input and outputs a probability for each of the three regulatory outcomes.
    ...
    st.markdown(f"""
        ...
        Each cluster groups breaches that share characteristics such as data types involved, incident type, breach scale and reporting time.

        An important finding from the modelling work: these clusters do **not** meaningfully align with regulatory outcomes (Adjusted Rand Index = {kmeans_summary.get("ari", 0.0):.3f}).
        This means that breaches similar in shape do not necessarily share regulatory decisions, the cluster information tells you "what kinds of breaches look like this one historically," not "what decision is likely."
        The Simulator's cluster panel is framed accordingly.
        """
    )

    # Limitations (from markdown)
