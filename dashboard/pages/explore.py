...
# Multi-valued features are comma-separated string cells
...
def _quarter_period_order(df: pd.DataFrame) -> List[str]:
    """
    Build the chronological order of year, quarter labels present in the filtered data, formatted as year quarter
    """
    pairs = (
...
def main() -> None:
    render_header(
        ...
        subtitle="Browse the previous UK healthcare data breach incidents the model is trained on.",
    )
    render_disclaimer()
    ...
    st.caption(
        "Each bar shows the proportion of regulatory decisions for breaches with that feature value. "
        "Bars sum to 100% within each value, so tall sections of one colour indicate that feature value's characteristic decision pattern."
    )
    ...
    for feature_col, feature_label in _FEATURE_PANELS:
        ...
        df_for_feature = (_explode_multilabel(df, feature_col) if feature_col in _MULTILABEL_COLS else df) 
