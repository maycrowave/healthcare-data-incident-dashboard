...
# We surface the three most distinctive ones rather than all six, to keep the description short.
...
def _describe_cluster(cluster_data: Dict) -> str:
    """
    Build a short plain-language description of a cluster from its
    dominant feature values.

    Picks the top value for each of the three most distinctive features
    and renders them as a sentence.
    """
    ...
    for feature in _DESCRIPTION_FEATURES[:3]:
        ...
        # top_features values look like {"Non Cyber": 71.3, ...}
        # Take the highest-percentage value.
...
def render_cluster_panel(inputs: Dict[str, object]) -> None:
    ...
    # Persistent framing every render. F4 requires this visibility.
    st.info(
        "**This is a similarity finding, not a prediction.** "
        "Clustering groups breaches by their characteristics. The decision "
        "distribution below shows what the ICO historically decided for "
        "breaches that look similar to yours — not what they would decide "
        "for your specific breach. Statistical analysis showed clusters do "
        "not meaningfully align with regulatory decisions."
    )
    ...
    with st.container(border=True):
        ...
        with header_right:
            ...
            pct = cluster_data.get("percentage_of_dataset", 0.0)
            st.markdown(
                f"<div style='text-align: right; color: var(--text-color); opacity: 0.7;'>"
                f"{size:,} breaches · {pct:.1f}% of dataset"
                f"</div>",
        ...
        # Convert from percentages (stored as 0-100) to proportions (0-1) for the chart.
        proportions = {
            label: outcome_distribution.get(label, 0.0) / 100.0
